...
void Matrix::ReadFromFile(const std::string filename)
{
   ...
   int n_file;
   status = H5Aread(attribute_id, H5T_NATIVE_INT, &n_file);
   HDF5_STATUS_CHECK(status);
   ...
   status = H5Dread(dataset_id, H5T_NATIVE_DOUBLE, H5S_ALL, H5S_ALL, H5P_DEFAULT, matrix.get());
   HDF5_STATUS_CHECK(status);
 ...
 */
void Matrix::solve(std::vector<double> &b)
{
   ...
   dgetrf_(&rows,&colums,matrix.get(),&lda2,ipiv.get(),&INFO);

   if(INFO)
      std::cerr << "Something went wrong in dgetrf: " << INFO << endl;

   dgetrs_(&trans,&n,&nrhs,matrix.get(),&lda,ipiv.get(),b.data(),&ldb,&INFO);
